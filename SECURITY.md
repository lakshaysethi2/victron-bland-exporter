...
This repository is public, so treat **site-identifying detail** the same way as a secret: real hostnames and tunnel URLs, device MACs, LAN addresses, the physical layout of the installation, and the owner's timezone or tariff. Describe the mechanism, not the address. The same rule binds contributors and coding agents working in this tree — see the privacy section of [`AGENTS.md`](AGENTS.md). Use `charger.example.com` and `AA:BB:CC:DD:EE:FF` in examples, tests, and screenshots.
