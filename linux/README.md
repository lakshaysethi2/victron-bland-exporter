...
This Victron feeds a downstream MPPT, not the cells. The watchdog pulses when the 2-minute average `Vpv − Vout` is near the 2-hour (max panel − max out) Voc gap, Victron out is 30–52 V (not a collapsed bus), and watts are below 0.85 × this hour’s clear-sky envelope. Each pulse costs a yield dip, so auto-pulse is limited to 15 minutes apart and 4 per hour (`local_mpp_max_per_hour` in `linux/yield_config.json`, or `MPPT_MAX_PULSES_PER_HOUR` in `~/.config/mppt/secrets.env`).
