...
While the app runs it also reads the solar **panel voltage** (register `0xEDBB` over the same GATT service, ~every 60 s) and exposes it as `victron_panel_voltage_volts` — the Instant Readout broadcast does not carry it. The gauge is served only while it is fresh (a device-answered `0xFFFF` — no panel voltage in darkness — or a value not refreshed within 5 minutes is omitted, never a stale number); `victron_panel_voltage_state` carries the reason (0=ok, 1=no MAC configured, 2=last read failed with the message in the `error` label, 3=no value — never read or device answered NA, 4=stale) so a silent GATT failure is visible on `/metrics` without needing the debug log.
