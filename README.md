...
- 🌐 **Remote charger + voltage control** — flip the charger, set voltages, paste an Instant Readout key, or save/start/stop the named Cloudflare tunnel from any browser at `https://mppt.lak.nz/` (`/charger` and `/voltage` also work) or `http://<phone-ip>:5338/` (LAN), protected by a shared secret you set in the app; the control page also shows whether the named tunnel is up so a LAN visit can restore the public bridge
