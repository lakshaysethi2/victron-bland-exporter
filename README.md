...
- 🌐 **Remote charger + voltage control** — flip the charger, set voltages, or paste an Instant Readout key from any browser at `https://mppt.lak.nz/charger` and `https://mppt.lak.nz/voltage` (named tunnel) or `http://<phone-ip>:5338/...` (LAN), protected by a shared secret you set in the app; the control page also shows whether the named tunnel is up so a LAN visit can tell you if the phone is still bridging
